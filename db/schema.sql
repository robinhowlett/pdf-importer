-- Handycapper schema for pdf-importer
-- Applied automatically by Docker Compose (docker-entrypoint-initdb.d).
-- For manual setup: psql -U handycapper -d handycapper -f db/schema.sql
...
(
  ...
  scheduled_surface   text,
  scheduled_course    text,
  off_turf            boolean,
  format              varchar(5),
